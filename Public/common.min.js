...
SML.TreeSelector = function(item,data,rootId,defaultValue){ 
    this._data = data; 
    this._item = item; 
    this._rootId = rootId; 
    if(defaultValue)this.defaultValue = defaultValue;
} 
SML.TreeSelector.prototype.createTree = function(){ 
...
SML.TreeSelector.prototype.createSubOption = function(len,current,next){ 
    var blank = ".."; 
    ...
    SML.TreeSelector.prototype.getLevel = function(datasources,topId,currentitem){ 
 ...
 * 获取版本
 */
SML.getSMLMAllVersion = function(url){
	$.post(url,{},function(data,textStatus){
	   ...
	   if(json){
		   if(json.version && json.version!='same'){
			   $('.SMLmall-version-tips').show();
			   $('#SMLmall_version').html(json.version);
			   $('#SMLmall_down').attr('href',json.downloadUrl);
		   }
		   if(json.accredit=='no'){
			   $('.SMLmall-accredit-tips').show();
		   }
 ...
 * 取文档内容实际高度  
 *******************/  
 SML.getScrollHeight = function()  
 {  
     return Math.max(document.body.scrollHeight,document.documentElement.scrollHeight);  
 }

 //只能輸入數字
 SML.isNumberKey = function(evt){
 ...
 //只能輸入數字和小數點
 SML.isNumberdoteKey = function(evt){
 ...
 //只能輸入數字和字母
 SML.isNumberCharKey = function(evt){
 	var e = evt || window.event; 
 	var srcElement = e.srcElement || e.target;	
 	var charCode = (evt.which) ? evt.which : event.keyCode;
...
SML.isChinese = function(obj,isReplace){
 	var pattern = /[\u4E00-\u9FA5]|[\uFE30-\uFFA0]/i
...
//比较时间差
SML.getTimeDiff = function(startTime,endTime,diffType){
 ...
 * @returns
 */
SML.checkMinLength = function(o,minLength){
	if(SML.getBytes(o)<=minLength){
		return false;
	}
	return true;
}
 ...
 * @returns
 */
SML.checkMaxLength = function(o,maxLength){
	if(SML.getBytes(o)>maxLength){
		return false;
	}
	return true;
}
...
SML.toJson = function(str,notLimit){
	var json = {};
	try{
...
//把SML-panel-full样式的表单设置布满屏幕高度
$(function () {
	if($('.SML-panel-full').height()<SML.pageHeight())$('.SML-panel-full').height(SML.pageHeight()-3);
});
 ...
 * 替换url
 */
SML.replaceURL = function(url,ar){
...
SML.checkChks = function(obj,cobj){
	$(cobj).each(function(){
		$(this)[0].checked = obj.checked;
	})
}
SML.getChks = function(obj){
	var ids = [];
	$(obj).each(function(){
...
SML.blank = function(str,defaultVal){
...
SML.tips = function(content, selector, options){
	var opts = {};
	opts = $.extend(opts, {tips:1, time:2000, maxWidth: 260}, options);
	return layer.tips(content, selector, opts);
}
SML.open = function(options){
	var opts = {};
	opts = $.extend(opts, {}, options);
	return layer.open(opts);
}
SML.limitDecimal = function(obj,len){
	var s = obj.value;
 	if(s.indexOf(".")>-1){
