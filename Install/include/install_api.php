...
if($act=='list'){
	...
	$sql = "../data/".$db_type."/sml_".$_POST['table'].".sql";
	$sql = file_get_contents($sql);
	runquery($sql,$db_prefix);
	if(intval($_POST['isFinish'])==1){
		if(intval($admin_name)==11){
			$sql = 'UPDATE '.$db_prefix.'users SET tel="'.$admin_name.'",password="'.md5($admin_password."huazhongsmile").'" WHERE id="1001"';
		}else{
			$sql = 'UPDATE '.$db_prefix.'users SET qq="'.$admin_name.'",password="'.md5($admin_password."huazhongsmile").'" WHERE id="1001"';
		}
	    runquery($sql,$db_prefix);
	    initConfig($db_host,$db_user,$db_port,$db_pass,$db_prefix,$db_name);
	    if(!file_exists(INSTALL_ROOT."/Application/common/conf/config.php")){
	    	echo json_encode(array('status'=>-1,'msg'=>'无法创建配置文件，请检查Application/common/conf目录是否有写入权限!'));exit();
	    }
